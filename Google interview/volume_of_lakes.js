...
const vol = arr => {
  ...
  while (leftIndex < rightIndex) {
    if (arr[leftIndex] <= arr[rightIndex]) {
      ...
      trap += Math.max(0, leftMax - arr[leftIndex])
      leftIndex++;
    } else {
      rightMax = Math.max(rightMax, arr[rightIndex]);
      trap += Math.max(0, rightMax - arr[rightIndex])
      rightIndex--;
    }
  }
...
console.log(vol( [1,3,2,4,1,3,1,4,5,2,2,1,4,2,2]));

// Dijkstra algorithm is used to find volume